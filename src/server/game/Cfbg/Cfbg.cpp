...
bool Player::SendRealNameQuery()
{
    ...
    data << uint8(getORace());
    data << uint8(GetByteValue(PLAYER_BYTES_3, 0));   // save gender from PLAYER_BYTES_3, UNIT_BYTES_0 changes with every transform effect
    data << uint8(GetClass());
...
void Player::FitPlayerInTeam(bool action, Battleground* pBattleGround)
{
    ...
    if (pBattleGround && action)
        SendChatMessage("%sYou are playing with the team %s%s in the %s", MSG_COLOR_WHITE, GetTeam() == ALLIANCE ? MSG_COLOR_DARKBLUE"BLUE" : MSG_COLOR_RED"RED", MSG_COLOR_WHITE, pBattleGround->GetName().c_str());
}
